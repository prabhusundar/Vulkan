...
layout (binding = 3, std140) uniform _u_bufferBoneTransform {
        mat4 modelMatrix;
...
layout (location = 1) out vec3 v_f_bitangent;
layout (location = 2) out vec3 v_f_tangent;
...
void main(void)
{
	...
	v_f_normal = mat3(u_bufferViewProjection.viewMatrix) * u_bufferBoneTransform.modelNormalMatrix * normal;
    v_f_bitangent = mat3(u_bufferViewProjection.viewMatrix) * u_bufferBoneTransform.modelNormalMatrix * bitangent;
    v_f_tangent = mat3(u_bufferViewProjection.viewMatrix) * u_bufferBoneTransform.modelNormalMatrix * tangent;

	v_f_texCoord = a_texCoord;

    vec4 position = u_bufferViewProjection.viewMatrix * u_bufferArmatureTransform.modelMatrix * vertex;
