...
layout (location = 0) in vec3 v_g_normal[];
layout (location = 1) in vec3 v_g_bitangent[];
layout (location = 2) in vec3 v_g_tangent[];
layout (location = 3) in vec2 v_g_texCoord[];


layout (location = 0) out vec4 v_aabb;
layout (location = 1) out flat float v_orientation;
...
void main(void)
{
    ...
    {
        vertex[i] = u_bufferViewProjection.projectionMatrix * u_bufferViewProjection.viewMatrix * u_bufferTransform.modelMatrix * gl_in[i].gl_Position;
        vertex[i] /= vertex[i].w;
    ...
    v_aabb = aabb + vec4(-u_bufferParameter.halfPixelSize, u_bufferParameter.halfPixelSize);
    ...
    for (i = 0; i < gl_in.length(); i++)
    {
		...
		plane[i].z -= dot(u_bufferParameter.halfPixelSize, abs(plane[i].xy));
    }
    ...
    for (i = 0; i < gl_in.length(); i++)
    {
        ...
        v_normal = mat3(u_bufferViewProjection.viewMatrix) * u_bufferTransform.normalMatrix * v_g_normal[i];
        v_bitangent = mat3(u_bufferViewProjection.viewMatrix) * u_bufferTransform.normalMatrix * v_g_bitangent[i];
        v_tangent = mat3(u_bufferViewProjection.viewMatrix) * u_bufferTransform.normalMatrix * v_g_tangent[i];
